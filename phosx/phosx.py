...
def ks_statistic(
    ...
    if plot_bool:
        ...
        sns.despine()
        plt.tight_layout()
        plt.savefig(f'{out_plot_prefix_str}{kinase}.svg')
        plt.close()
...
def compute_ks_pvalues(
    ...
    for kinase_str in ks_empirical_distrib_df.columns:
        ...
        if plot_bool:
            ...
            sns.despine()
            plt.tight_layout()
            plt.savefig(f'{out_plot_prefix_str}{kinase_str}_ks_dist.svg')
            plt.close()
...
def compute_kinase_activity(
    ...
    ks_series = seqrnk_pssm_scoring_row_scaled_df.apply(
        ks_statistic,
        args=[seqrnk_pssm_null_scoring_df, plot_figures, out_plot_prefix],
        axis=0
    )
    ks_series.name = 'KS'
    # compute ks pvalues
    ...
    activity_score_series = results_df['p-value']
